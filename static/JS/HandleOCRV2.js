...
function ProcessSubmission(DataURL, Count) {
  Tesseract.recognize(`${DataURL}`, "eng", { logger: m => LogPercent(m) }).then(
    ({ data: { text } }) => {
  ...
  $(function () {
    try {
      ...
      var basic = new Croppie(Element, {
        viewport: {
          width:
            ResolutionArray[ResolutionSelection].Viewport[CroppieCounter].X,
          height:
            ResolutionArray[ResolutionSelection].Viewport[CroppieCounter].Y,
        },
